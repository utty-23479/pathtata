...
const LogIn = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(newErrors).length === 0) {
            ...
            // --- LÓGICA CON LOCAL STORAGE (JSON) ---
            const users = JSON.parse(localStorage.getItem("users") || "[]");
            const userFound = users.find(u => u.email === loginData.email && u.password === loginData.password);

            if (userFound) {
                // Guardamos el usuario actual para usar su ID en el Dashboard después
    ...
    return (
        <>
            <Header />
            <div className="title-center"><h1>Login</h1></div>
            <div className="container-form">
                <form onSubmit={handleSubmit}>
                    <Input type="email" label="email" text="Email" name="email" placeHolder="Your email" value={loginData.email} classes="font-bold" classesInput="input-form" onChange={handleChange} />
                    {errors.email && <p className="error-message">{errors.email}</p>}
                    <Input type="password" label="password" text="Password" name="password" placeHolder="Password" value={loginData.password} classes="input-form font-bold" classesInput="input-form" onChange={handleChange} />
                    {errors.password && <p className="error-message">{errors.password}</p>}
                    <TextButton colorText="white" buttonText={LOGIN_TEXT} isSubmit={true} isLoading={isLogging} fontWeight="bold" myClasses='h-md-button gradient-tata' />
                    <div className="text-center mt1">
                        <p>If you don't have an account you may <Link className="link" to="/signUp">Registration</Link></p>
                    </div>
                </form>
